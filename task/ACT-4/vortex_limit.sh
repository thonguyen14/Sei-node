...
#seid tx sign $HOME/SEI_ACT4/gen_limit_tx.json -s $seq -a $ACC --offline --from $wallet --chain-id atlantic-1 --output-document $HOME/SEI_ACT4/limit_tx.json --node https://sei-testnet-rpc.polkachu.com:443; sleep 7;
echo abcdef | seid tx sign $HOME/SEI_ACT4/gen_limit_tx.json -s $seq -a $ACC --offline --from $wallet --chain-id atlantic-1 --output-document $HOME/SEI_ACT4/limit_tx.json --node https://sei-testnet-rpc.polkachu.com:443; sleep 10;

#date -R >> $HOME/SEI_ACT4/txh.log
echo "Limit order:" >> $HOME/SEI_ACT4/txh.log
seid tx broadcast $HOME/SEI_ACT4/limit_tx.json --node https://sei-testnet-rpc.polkachu.com:443 | grep txhash >> $HOME/SEI_ACT4/txh.log
